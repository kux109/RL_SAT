...
class CDCLSolverBaseline(CDCLCore):
    def luby(self, i: int) -> int:
        ...
        while True:
            if i == (1 << k) - 1:
                return 1 << (k - 1)
            # Find which power of 2 interval i falls into
            # if i is between 2^(k-1)-1 and 2^k-1
            p = (1 << (k - 1))
            if i >= p - 1 and i < (1 << k) - 1:
                 # recursive call: luby(i - (2^(k-1)) + 1)
                 # wait, the formula is:
                 # luby(i) = 1 if i = 2^k - 1
                 # luby(i) = luby(i - 2^(k-1) + 1) otherwise
                 # let's use a simpler iterative approach or just the standard recursive one
                 # In practice for 1-based index i:
                 # if i = 2^k - 1, output 2^(k-1)
                 # else output luby(i - 2^(k-1) + 1)
                 # We can just use the iterative generator I wrote below which is simpler
                 pass
            k += 1
            if k > 30: # safety
                return 1

    def make_luby_sequence(self):
        # Generator for luby sequence: 1 1 2 1 1 2 4 1 1 2 1 1 2 4 8 ...
        # logic:
        # luby[i] = 1 if i = 2^k - 1
        # luby[i] = luby[i - 2^(k-1) + 1] otherwise
        # but 1-indexed.
    ...
    def __init__(self, num_vars: int, clauses: List[List[int]], heuristic: str = "vsids"):
        ...
        # Luby restarts
        self.luby_gen = self.make_luby_sequence()
        self.base_restart_limit = 100
        self.current_restart_limit = self.base_restart_limit * next(self.luby_gen)
        self.conflicts_at_last_restart = 0
    ...
    def maybe_restart(self):
        if self.conflicts - self.conflicts_at_last_restart >= self.current_restart_limit:
            if self.decision_level > 0:
                self.backtrack(0)
                self.restarts += 1
                self.conflicts_at_last_restart = self.conflicts
                self.current_restart_limit = self.base_restart_limit * next(self.luby_gen)
    ...
    def solve(self) -> bool:
        ...
        while True:
            ...
            if conflict is not None:
                self.conflicts += 1
                now = time.time()
                if self.conflicts % 100 == 0 or now - last_log > 2:
                    # Update log less frequently to standard out? kept same logic
    ...
    def _log_epoch(self, final: bool = False):
        ...
        feature_names = [
            'feat_avg_lbd', 'feat_var_lbd', 'feat_glue_ratio', 'feat_conf_rate',
            'feat_act_mean', 'feat_act_std', 'feat_learned_ratio', 'feat_cv_ratio',
            'feat_rest_rate', 'feat_prop_rate', 'feat_sat_ratio'
        ]
        for i, v in enumerate(ctx):
            if i < len(feature_names):
                row[feature_names[i]] = v
            else:
                row[f'feat_{i}'] = v
        self.logger.log(row)
        self.epoch_index += 1
