...
def run_solver(binary_path, cnf_file, use_rl=False, timeout=30, step=500):
    cmd = [binary_path, cnf_file, "--step", str(step)]
    if use_rl:
        cmd.append("--rl")
...
def main():
    ...
    parser.add_argument("--output", default="benchmark_results.txt", help="Output log file")
    parser.add_argument("--limit", type=int, default=0, help="Limit number of instances (0 = all)")
    parser.add_argument("--step", type=int, default=500, help="Epoch step size for RL")
    ...
    print(f"Found {len(cnf_files)} instances. Running benchmark (Timeout: {args.timeout}s, Step: {args.step})...")
    ...
    for f in cnf_files:
        ...
        # Run Baseline
        st1, t1, c1, r1, l1, g1 = run_solver(args.solver, f, use_rl=False, timeout=args.timeout, step=args.step)
        print(header_fmt.format(basename, "BL", st1, f"{t1:.4f}", c1, r1, f"{l1:.2f}", f"{g1:.2f}"))
        
        # Run RL
        st2, t2, c2, r2, l2, g2 = run_solver(args.solver, f, use_rl=True, timeout=args.timeout, step=args.step)
    ...
    for r in results:
        ...
        # Only count valid wins (not timeouts/errors, unless one timed out and other didn't)
        if r['bl'][0] != "SAT" and r['rl'][0] == "SAT":
            rl_wins += 1
        elif r['bl'][0] == "SAT" and r['rl'][0] != "SAT":
            bl_wins += 1
        elif t1 < t2:
            bl_wins += 1
        elif t2 < t1:
    ...
    print("-" * 90)
    print(f"Summary over {len(results)} instances:")
    print(f"Baseline Wins: {bl_wins}")
    ...
    with open(args.output, "w") as f:
        ...
        f.write(f"\nSummary:\n")
        f.write(f"Baseline Wins: {bl_wins}\n")
        f.write(f"RL Wins:       {rl_wins}\n")
        f.write(f"Ties:          {ties}\n")
